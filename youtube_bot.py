...
def generate_content(headlines, language_name, gemini_lang):
    ...
    2. A YouTube Title (Max 60 chars) that creates a MASSIVE CURIOSITY GAP. Do not just state the news; make the viewer NEED to click to find out what happens next.
...
def assemble_long_video(audio_file, ticker_text, lang_name, headlines, font_path, image_paths):
    ...
    for i, img_path in enumerate(image_paths):
        ...
        if i < num_imgs - 1: clip = clip.fx(fadeout, 0.5)
    ...
    if os.path.exists("my_photo.png"):
        ...
        print("⚠️ my_photo.png not found. Skipping anchor overlay.")
    ...
    ticker_speed = total_distance / duration
    ticker_clip = ticker_clip.set_position(lambda t: (1920 - ticker_speed * t, 1010))
    ...
    layers.extend([logo, live_dot, live_text, clock, ticker_bg, ticker_label, ticker_clip] + lower_thirds)
...
def assemble_shorts(short_script, lang_name, tts_voice, font_path, image_paths):
    ...
    bg_img = ImageClip(image_paths[0]).fx(colorx, 0.6).resize((1080, 1920))
    bg = bg_img.set_duration(duration)
...
# --- 7. PRO AUTO-THUMBNAIL GENERATOR ---
